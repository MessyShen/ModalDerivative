...
#include "LoboDynamic/materials/TypeNeoHookeanMaterial.h"
#include "LoboDynamic/materials/TypeStVKMaterial.h"
...
Lobo::HyperelasticModel::HyperelasticModel(Lobo::LoboTetMesh *tetmesh_)
    : tetmesh(tetmesh_)
{
...
void Lobo::HyperelasticModel::setMaterial(const std::string material_type, 
    ...
    if (strcmp("StVK", material_type.c_str()) == 0)
    {
        this->elastic_material =
            new TypeStVKMaterial<double>(tetmesh, 0, 500.0);
        materialtype = "StVK";
    }
    if (strcmp("NeoHookean", material_type.c_str()) == 0)
    {
        this->elastic_material =
...
void Lobo::HyperelasticModel::computeReducedEnergy(
    Eigen::VectorXd *free_variables, double *energy, Eigen::VectorXd *fullspace_f,
    Eigen::SparseMatrix<double> *hessian, std::vector<int> *w_indices,
  ...
  if (computationflags & Computeflags_reset) {
    ...
    if (computationflags & Computeflags_second) {
      for (int i = 0; i < hessian->outerSize(); ++i)
        for (Eigen::SparseMatrix<double>::InnerIterator it(*hessian, i); it;
             ++it) {
          it.valueRef() = 0;
        }
    }
  ...
  int num_cubature = w_indices->size();

  for (int i = 0; i < num_cubature; i++) {
    int idx = w_indices->operator[](i);
    this->getTetForceMatrixAnalytical(idx, internalforce_list[idx],
                                      stiffness_list[idx], computationflags);
    for (int ni = 0; ni < 4; ++ni) {
      int n = tetmesh->tet_indices[idx * 4 + ni];
      for (int nj = 0; nj < 3; ++nj) {
        // ni * 3 + nj
        int idx_in_full = n * 3 + nj;
        /*std::cout << ni << " " << nj;
        std::cout << " index in full " << idx_in_full;
        std::cout << " idx " << idx;
        std::cout << std::endl;*/
        /*
         for (int i = 0; i < 4; i++)
        ...
        */
        fullspace_f->data()[idx_in_full] +=
              weights->data()[i] * internalforce_list[idx]->data()[ni * 3 + nj];
    ...
    for (int c = 0; c < 4; c++) {
      for (int a = 0; a < 4; a++) {
        int row = tetmesh->tet_indices[idx * 4 + c] * 3;
        int col = tetmesh->tet_indices[idx * 4 + a] * 3;
        ...
        for (int r = 0; r < 3; r++) {
          for (int l = 0; l < 3; l++) {
            ...
            double target = stiffness_list[idx]->data()[block_c * 12 + block_r];

            int index =
                column_[idx][3 * numElementVertices * block_c + block_r];

            hessian->valuePtr()[index] +=
                weights->data()[i] *
                stiffness_list[idx]->data()[block_c * 12 + block_r];
          }
        }
      }
    }
...
void Lobo::HyperelasticModel::computeReducedEnergy(
    Eigen::VectorXd *free_variables, double *energy, Eigen::VectorXd *reduce_f,
    Eigen::SparseMatrix<double> *hessian, 
    std::vector<int> *w_indices,
    Eigen::VectorXd *weights,
    Eigen::MatrixXd *projection, int computationflags) {
  currentdisplacement = *free_variables;
  if (computationflags & Computeflags_reset) {
    if (computationflags & Computeflags_energy)
      *energy = 0;
  ...
  int num_ele = tetmesh->getNumElements();
  int num_cubature = w_indices->size();
  int full_dof = projection->rows();
  int reduce_dof = projection->cols();
  ...
  for (int i = 0; i < num_cubature; i++) {
    ...
    for (int ni = 0; ni < 4; ++ni) {
      ...
      for (int nj = 0; nj < 3; ++nj) {
        ...
        for (int r = 0; r < reduce_dof; ++r) {
          reduce_f->data()[r] += weights->data()[i] *
                                 projection->operator()(idx_in_full, r) *
                                 internalforce_list[idx]->data()[ni * 3 + nj];
      ...
      for (int a = 0; a < 4; a++) {
        ...
        for (int r = 0; r < 3; r++) {
          for (int l = 0; l < 3; l++) {
            ...
            int index = column_[idx][3 * numElementVertices * block_c + block_r];
...
#pragma omp for
    ...
    //std::cout << "HERE INTERNALFORCE OF 6th ELEMENT\n";
    //std::cout << internalforce_list[6] << std::endl;
    //std::cout << stiffness_list[6] << std::endl;
    ...
    //std::cout << "jacobi" << jacobi->transpose() << std::endl;
    //std::cout << std::endl;
}
...
void Lobo::HyperelasticModel::computeEnergySparse(
    Eigen::VectorXd *free_variables, double *energy, Eigen::VectorXd *jacobi,
    Eigen::SparseMatrix<double> *hessian, std::vector<int> constmap) {
  currentdisplacement = *free_variables;
  *energy = 0;
  ...
  int comp_flag = 0;
  comp_flag = (Computeflags_fisrt | Computeflags_second);
  for (int i = 0; i < num_ele; i++) {
    this->getTetForceMatrixAnalytical(i, internalforce_list[i],
                                      stiffness_list[i], comp_flag);
  }
  for (int i = 0; i < num_ele; i++) {
    assignTetElementForceAndMatrix(i, energy, jacobi, hessian, comp_flag,
                                   1.0);
  }
...
void Lobo::HyperelasticModel::computeFq_Gq_full(
    Eigen::VectorXd &full_u,  Eigen::VectorXd &f_u,
    Eigen::MatrixXd &g_u, int computationflags){
  /*
  f_q : q dimension reduce internal force
  g_q: n_element x u mat
  */
  //Eigen::VectorXd full_u = proj * reduce_q;
  currentdisplacement = full_u;
  int full_dof = full_u.size();
  int num_ele = tetmesh->getNumElements();

  
  g_u.resize(num_ele, full_dof);
  g_u.setZero();
  ...
  // assign g_q
  for (int eleid = 0; eleid < num_ele; ++eleid) {
    for (int i = 0; i < 4; i++) {
      int n = tetmesh->tet_indices[eleid * 4 + i];
      for (int j = 0; j < 3; ++j){
        g_u(eleid, n * 3 + j) = internalforce_list[eleid]->data()[i * 3 + j];
        f_u(n * 3 + j) += internalforce_list[eleid]->data()[i * 3 + j];
      }
    }  
  }
...
void Lobo::HyperelasticModel::getTetForceMatrixAnalytical(int eleid, Eigen::VectorXd *internalforce, Eigen::MatrixXd *stiffness, int computationflags)
{
    ...
    if (isinvertible)
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                std::cout << "trigger invertible" << std::endl;
    ...
    /*if (eleid == 0) {
      std::cout << "here P at 0" << P << std::endl;
      std::cout << "f_hat at 0" << fhat << std::endl;
      std::cout << "_dF_du at 0" << dF_du[0] << std::endl;
      std::cout << "te->volume" << te->volume << std::endl;
    }*/
   ...
   /* if (eleid == 0) {
      std::cout << "phat" << phat[0] << " " << phat[1] << " " << phat[2] << " "
                << "fhat" << fhat[0] << " " << fhat[1] << " " << fhat[2] << " "
                << std::endl;
      std::cout << "computationflags" << computationflags << std::endl;
      std::cout << "m_U" << m_U << std::endl;
      std::cout << "m_V" << m_V << std::endl;
      std::cout << "P" << P << std::endl;
      std::cout << "dF_du[eleid]" << dF_du[eleid] << std::endl;
      std::cout << "   internal force" << eleid << " "
                << internalforce->transpose() << std::endl;
    }*/
    if (computationflags & Computeflags_second)
    {
...
{
    //std::cout << "before assiagn"<< std::endl;
    //std::cout << MatrixXd(*sparseMatrix) << std::endl;
            ...
            for (int a = 0; a < 4; a++)
            {
                ...
                for (int r = 0; r < 3; r++)
                {
                    for (int l = 0; l < 3; l++)
                    {
                        ...
                        double target = stiffness_list[eleid]
    ...
    //std::cout << "assign " << *energy << " flag: " << flags << std::endl;
    //std::cout << *internalforce << std::endl;
}
...
void Lobo::HyperelasticModel::ComputeDiagonalPFromStretches(int eleid, double *lambda, double *PDiag, double &energy)
{
    ...
    //std::cout << PDiag << std::endl;
    /*if (eleid == 0) {
      std::cout << "mat " << mat << std::endl;
      std::cout << "dPsidIV " << dPsidIV << std::endl;
