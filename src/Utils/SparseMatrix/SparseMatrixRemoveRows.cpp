...
void subSparseMatrix(SparseMatrix<double> &source, SparseMatrix<double> &result, std::vector<int> &map)
{
  ...
  std::vector<EIGEN_TRI> removecoef;
	if (map.size() == source.rows())
	{
		for (int j = 0; j < source.outerSize(); ++j)
          ...
          removecoef.push_back(EIGEN_TRI(i_p, j_p, it.value()));
				}
			}
...
void VectorRemoveRows(std::vector<int> &map, VectorXd &target, VectorXd &result, int numConstrainedDOFs_ , int* constrainedDOFs_ )
{
...
void VectorInsertRows(std::vector<int> &map, VectorXd &target, VectorXd &result, int numConstrainedDOFs_ , int* constrainedDOFs_ )
{
	result.setZero();
...
void createReverseMapByConstrains(std::vector<int> &map, 
																	std::vector<int> &reverseMap,
																	int r,
                                  int numConstrainedDOFs_,
                                  std::vector<int> &constrainedDOFs_) {
  map.resize(r);
	reverseMap.resize(r - numConstrainedDOFs_);
  std::fill(map.begin(), map.end(), 0);
  for (int i = 0; i < numConstrainedDOFs_; i++) {
    map[constrainedDOFs_[i]] = -1;
  }
...
void createSparseMapbyTopology(SparseMatrix<double>* sparseMatrix, SparseMatrix<double>* subsparseMatrix, std::vector<int>& entry_map, std::vector<int>& rowmap, int r, int numConstrainedDOFs_, int* constrainedDOFs_)
{
	...
	std::vector<Triplet<double>> subcoef;
	...
	for (int j = 0; j < sparseMatrix->outerSize(); ++j)
		for (SparseMatrix<double>::InnerIterator it(*sparseMatrix, j); it; ++it)
		{
			...
			if (!(i_p == -1 || j_p == -1))
			{
				subcoef.push_back(Triplet<double>(i_p, j_p, it.value()));
			}
		}
